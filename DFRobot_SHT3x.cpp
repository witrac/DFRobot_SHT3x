 ...
 * @brief Define the infrastructure and the implementation of the underlying
 * method of the DFRobot_SHT3x class,
 *
...
void DFRobot_SHT3x::init() { begin(); }

float DFRobot_SHT3x::readTemperature() { return getTemperatureC(); }

float DFRobot_SHT3x::readHumidity() { return getHumidityRH(); }

bool DFRobot_SHT3x::measure_is_ok(float measure) {
  if (measure == ERROR_BAD_CRC) {
    return false;
  } else {
    return true;
  }
...
uint32_t DFRobot_SHT3x::readSerialNumber() {
  ...
  if ((checkCrc(serialNumber1) == serialNumber1[2]) &&
      (checkCrc(serialNumber2) == serialNumber2[2])) {
...
DFRobot_SHT3x::readTemperatureAndHumidity(eRepeatability_t repeatability) {
  uint8_t rawData[6];
...
bool DFRobot_SHT3x::startPeriodicMode(eMeasureFrequency_t measureFreq,
                                      eRepeatability_t repeatability) {
  const uint16_t cmd[5][3] = {
      {SHT3X_CMD_SETMODE_H_FREQUENCY_HALF_HZ,
       SHT3X_CMD_SETMODE_M_FREQUENCY_HALF_HZ,
       SHT3X_CMD_SETMODE_L_FREQUENCY_HALF_HZ},
      {SHT3X_CMD_SETMODE_H_FREQUENCY_1_HZ, SHT3X_CMD_SETMODE_M_FREQUENCY_1_HZ,
      ...
      {SHT3X_CMD_SETMODE_H_FREQUENCY_4_HZ, SHT3X_CMD_SETMODE_M_FREQUENCY_4_HZ,
       SHT3X_CMD_SETMODE_L_FREQUENCY_4_HZ},
      {SHT3X_CMD_SETMODE_H_FREQUENCY_10_HZ, SHT3X_CMD_SETMODE_M_FREQUENCY_10_HZ,
       SHT3X_CMD_SETMODE_L_FREQUENCY_10_HZ}};
  sStatusRegister_t registerRaw;
...
uint8_t DFRobot_SHT3x::environmentState() {
  ...
  if (registerRaw.humidityAlert == 1 && registerRaw.temperatureAlert == 0) {
    if (data.Humidity > rhHighSet)
      ...
      return 1;
  } else if (registerRaw.humidityAlert == 0 &&
             registerRaw.temperatureAlert == 1) {
    if (data.TemperatureC > tempHighSet)
      return 20;
    else if (data.TemperatureC < tempLowSet)
      return 10;
  } else if (registerRaw.humidityAlert == 1 &&
             registerRaw.temperatureAlert == 1) {
...
uint8_t DFRobot_SHT3x::setTemperatureLimitC(float highset, float highclear,
                                            float lowset, float lowclear) {
...
uint8_t DFRobot_SHT3x::setTemperatureLimitF(float highset, float highclear,
                                            float lowset, float lowclear) {
...
uint8_t DFRobot_SHT3x::setHumidityLimitRH(float highset, float highclear,
                                          float lowset, float lowclear) {
  uint16_t _highset, _highclear, _lowclear, _lowset, limit[1];
  if (highset > highclear && highclear > lowclear && lowclear > lowset) {
...
float DFRobot_SHT3x::getTemperatureHighSetF() { return limitData.highSet; }
float DFRobot_SHT3x::getTemperatureHighClearF() { return limitData.highClear; }
float DFRobot_SHT3x::getTemperatureLowClearF() { return limitData.lowClear; }
float DFRobot_SHT3x::getTemperatureLowSetF() { return limitData.lowSet; }
...
float DFRobot_SHT3x::getTemperatureHighSetC() { return limitData.highSet; }
float DFRobot_SHT3x::getTemperatureHighClearC() { return limitData.highClear; }
float DFRobot_SHT3x::getTemperatureLowClearC() { return limitData.lowClear; }
float DFRobot_SHT3x::getTemperatureLowSetC() { return limitData.lowSet; }
float DFRobot_SHT3x::getHumidityHighSetRH() { return limitData.highSet; }
float DFRobot_SHT3x::getHumidityHighClearRH() { return limitData.highClear; }
float DFRobot_SHT3x::getHumidityLowClearRH() { return limitData.lowClear; }
float DFRobot_SHT3x::getHumidityLowSetRH() { return limitData.lowSet; }
